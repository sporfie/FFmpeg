...
static int h264_abstimecode_update_fragment(AVBSFContext *bsf, AVPacket *pkt, CodedBitstreamFragment *au)
{
    ...
    tcd_buf = av_buffer_allocz(sizeof(H264AbsTimeCodeData));
    if (!tcd_buf)
    {
        av_log(bsf, AV_LOG_ERROR, "Failed to allocate memory for user data SEI message.\n");
        return AVERROR(ENOMEM);
    }
    tcd = (H264AbsTimeCodeData *)tcd_buf->data;
    ...
    if (prft)
    {
        ...
        av_log(bsf, AV_LOG_DEBUG, "From RTSP: pts %lld, epoch %lld, base: %lld, host time: %lld\n", (long long)pts_us, (long long)abs_time, (long long)(abs_time - pts_us), (long long)av_gettime());
    }
    ...
    err = ff_cbs_sei_add_message(ctx->common.output, au, 1, SEI_TYPE_USER_DATA_UNREGISTERED, &tcd->udu, tcd_buf);
    av_buffer_unref(&tcd_buf);
    if (err < 0)
    {
...
const AVBitStreamFilter ff_h264_abstimecode_bsf = {
    .name = "h264_abstimecode",
    .codec_ids = h264_abstimecode_codec_ids,
    .priv_class = &h264_abstimecode_class,
