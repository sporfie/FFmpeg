...
static int h265_abstimecode_update_fragment(AVBSFContext *bsf, AVPacket *pkt, CodedBitstreamFragment *au)
{
    ...
    tcd_buf = av_buffer_allocz(sizeof(H265AbsTimeCodeData));
    if (!tcd_buf)
    {
        av_log(bsf, AV_LOG_ERROR, "Failed to allocate memory for user data SEI message.\n");
        return AVERROR(ENOMEM);
    }
    tcd = (H265AbsTimeCodeData *)tcd_buf->data;
...
const AVBitStreamFilter ff_h265_abstimecode_bsf = {
    .name = "h265_abstimecode",
    .codec_ids = h265_abstimecode_codec_ids,
    .priv_class = &h265_abstimecode_class,
